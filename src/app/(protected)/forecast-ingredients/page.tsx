  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardHeader><CardTitle className="text-sm">Ingredients Checked</CardTitle></CardHeader>
      ...
      {data?.items_missing_recipes?.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              These menu items have predicted demand but no recipe lines, so ingredient needs can’t be calculated.
            </CardDescription>
          </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Suggested purchase keeps projected remaining stock at least the reorder level.
          </CardDescription>
        </CardHeader>
            ...
            <TableBody>
              {data?.ingredients?.map((x: any) => (
                <TableRow key={x.ingredient_id}>
                  <TableCell className="font-medium">{x.ingredient_name} <span className="text-xs text-muted-foreground">({x.unit})</span></TableCell>
              ...
              {!data?.ingredients?.length && (
                <TableRow>
                  <TableCell colSpan={8} className="text-sm text-muted-foreground">
                    No ingredient plan available (need recipes + inventory items).
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
